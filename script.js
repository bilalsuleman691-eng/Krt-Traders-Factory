...
function saveInvoiceNow() {
  ...
  if (editingInvTs) {
    ...
    if (idx > -1) {
      invoices[idx] = { ...invoices[idx], storeName, customerName: document.getElementById('inv-customer').value,
        ntn: document.getElementById('inv-ntn').value, strn: document.getElementById('inv-strn').value,
        address: document.getElementById('inv-address').value, date, items, discountPercent: disc,
        subTotal: sub.toFixed(2), discountAmt: discAmt.toFixed(2), finalTotal: final.toFixed(2) };
    ...
    invoices.push({
      ...
      date, items, discountPercent: disc,
...
function saveStockIn() {
  ...
  stockInEntries.push({
    srNo: Date.now(), date: document.getElementById('in-date').value,
    vendor: document.getElementById('in-vendor').value || 'N/A',
    itemName, barcode: document.getElementById('in-barcode').value || 'N/A',
...
function saveStockOut() {
  ...
  if (qty > bal) { alert(`Stock kam hai! Available: ${bal}`); return; }
  stockOutEntries.push({
    srNo: Date.now(), date: document.getElementById('out-date').value,
    customer, itemName, barcode: document.getElementById('out-barcode').value || 'N/A',
    qty, price, total: qty * price
  });
  saveAll();
  renderStockOutTable();
...
function addLedgerEntry(person) {
  ...
  data.push({
    id: Date.now(), date,
    credit: parseFloat(document.getElementById(person + '-credit').value) || 0,
    debit:  parseFloat(document.getElementById(person + '-debit').value)  || 0,
    note:   document.getElementById(person + '-note').value || ''
  });
...
function saveSPStockIn() {
  ...
  if (totalPcs <= 0) {
    alert('Ctn ya Extra Pcs mein qty likhein!');
    return;
  }

  spInEntries.push({
    srNo: Date.now(),
    date: document.getElementById('spin-date').value,
    vendor: document.getElementById('spin-vendor').value || 'N/A',
    itemName,
    barcode,
    pcsPerCtn,
    ctn,
  ...
  ['spin-item','spin-barcode','spin-ctn','spin-extra','spin-price']
...
function saveSPStockOut() {
  ...
  spOutEntries.push({
    srNo: Date.now(),
    date: document.getElementById('spout-date').value,
    store, barcode, itemName, qty, price, total: qty * price
  });
  saveAll();
  renderSPOutTable();
  updateSPStoreList();
