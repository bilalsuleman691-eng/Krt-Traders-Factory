...
function renderTaxInvoice(data) {
 ...
 container.innerHTML = taxInvoiceHTML; // your existing HTML
    
    // ============================================================
    // PRINT BUTTONS ADD KAREIN (container ke bahar)
    // ============================================================
    const buttonContainer = document.createElement('div');
    buttonContainer.style.cssText = 'text-align:center;margin-top:20px;display:flex;gap:10px;justify-content:center;flex-wrap:wrap;padding:15px 0;border-top:2px solid #e2e8f0;';
    buttonContainer.innerHTML = `
        <button class="btn btn-primary" onclick="printTaxInvoice()" style="padding:10px 28px;font-size:14px;border-radius:8px;cursor:pointer;background:#22c99a;color:#fff;border:none;">
            <i class="fas fa-print"></i> 🖨️ Print Tax Invoice
        </button>
        <button class="btn btn-success" onclick="shareWhatsAppTax()" style="padding:10px 28px;font-size:14px;border-radius:8px;cursor:pointer;background:#25D366;color:#fff;border:none;">
...
function printTaxInvoice() {
    ...
    if (taxInvoiceData) {
        ...
        // Fallback: print as HTML
        const w = window.open('', '_blank', 'width=1000,height=700');
        w.document.write(`<!DOCTYPE html><html><head><title>Tax Invoice</title>
        <style>
            body{font-family:Arial;padding:20px;background:#fff}
            .tax-invoice-display{max-width:1100px;margin:0 auto}
            .header{text-align:center;border-bottom:3px solid #22c99a;padding-bottom:14px}
            .header h1{color:#22c99a;font-size:24px}
            table{width:100%;border-collapse:collapse;margin:10px 0}
            th{background:#22c99a;color:#fff;padding:8px}
            td{padding:6px 8px;border-bottom:1px solid #ddd}
            .totals-grid{display:flex;justify-content:space-between;margin-top:15px;padding-top:15px;border-top:2px solid #ddd}
            .signature-section{display:flex;justify-content:space-between;margin-top:25px;padding-top:20px;border-top:2px solid #ddd}
            .sig-box{text-align:center;width:30%}
            .sig-line{border-top:2px solid #333;width:80%;margin:0 auto;padding-top:4px}
        ...
        </head><body>${content}<script>window.onload=function(){setTimeout(function(){window.print();},500);};<\/script></body></html>`);
        w.document.close();
    }
}
